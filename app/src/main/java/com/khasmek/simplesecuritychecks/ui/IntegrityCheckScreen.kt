...
import com.khasmek.simplesecuritychecks.checker.IntegrityChecker
...
@Composable
fun IntegrityCheckScreen(modifier: Modifier = Modifier) {
    val context = LocalContext.current
    val categories = remember { mutableStateListOf(*IntegrityChecker.getDefaultCategories().toTypedArray()) }
    var isChecking by remember { mutableStateOf(false) }
    var hasChecked by remember { mutableStateOf(false) }
    val scope = rememberCoroutineScope()

    val issuesDetected = hasChecked && categories.any { category ->
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        if (hasChecked && !isChecking) {
            Surface(
                color = if (issuesDetected) Color(0xFFD32F2F) else Color(0xFF388E3C),
                modifier = Modifier.fillMaxWidth()
            ) {
                Text(
                    text = if (issuesDetected) "INTEGRITY ISSUES DETECTED" else "DEVICE & APP INTEGRITY OK",
                    color = Color.White,
                    ...
                    onClick = {
                        ...
                        scope.launch {
                            val results = withContext(Dispatchers.IO) {
                                IntegrityChecker.runChecks(categories.toList(), context)
                            }
