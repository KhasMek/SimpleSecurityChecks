...
import com.khasmek.simplesecuritychecks.checker.CryptoChecker
...
@Composable
fun CryptoCheckScreen(modifier: Modifier = Modifier) {
    val categories = remember { mutableStateListOf(*CryptoChecker.getDefaultCategories().toTypedArray()) }
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        if (hasChecked && !isChecking) {
            Surface(
                ...
                Text(
                    text = if (allSucceeded) "ALL OPERATIONS SUCCEEDED" else "SOME OPERATIONS FAILED",
                    color = Color.White,
                    ...
                    onClick = {
                        ...
                        scope.launch {
                            val results = withContext(Dispatchers.IO) {
                                CryptoChecker.runChecks(categories.toList())
                            }
